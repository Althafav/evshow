...
export default function PressReleaseCarousel({ pageData }: any) {
  const items = pageData?.pressreleaseitems?.linkedItems || [];
  ...
  return (
    <section className="w-full">
      {/* Outer area like your screenshot */}
      <div className="relative rounded-3xl bg-[#06110A] px-4 py-5 sm:px-6">
        {/* Optional global arrows (remove if you don't want) */}
        <button
          onClick={scrollPrev}
          disabled={!canPrev}
          ...
          <div className="flex gap-6">
            {items.map((item: any) => {
              ...
              return (
                <div
                  key={item.system.id}
                  // Peek next slide like reference
                  ...
                  <div className="rounded-3xl bg-linear-to-r from-[#0096FF] via-[#07AFD2] to-[#1CFB4B] p-px">
                    <div className="flex overflow-hidden rounded-3xl bg-[#06110A]">
                      ...
                      <div className="relative flex-1 p-5">
                        {/* Top right circle arrow (like card action) */}
                        <div className="absolute right-4 top-4">
                          <a href={item?.elements?.link?.value || "#"} aria-label="Open">
                            <ArrowCircle />
                          </a>
                        </div>
                        ...
                        <h4 className="mt-1 pr-10 text-base line-clamp-1 font-semibold leading-snug text-[#1CFB4B]">
